...
DATA_DIR = os.environ.get("XAS_DATA_DIR", "801_Data")
...
def _load(scan_id: str) -> tuple:
    ...
    """
    # Resolve by exact name first, then by unique scan number (handles renamed
    # files like "27284 pristine.txt" referred to by number alone).
    try:
        fp = xu.scan_filepath(scan_id, DATA_DIR)
    except FileNotFoundError:
        # Also search in exported_data/ directory
        fp = xu.scan_filepath(scan_id, EXPORT_DIR)
    if fp not in _cache:
        ...
        _cache[fp] = {"meta": meta, "df": df}
    # Display id = the full descriptive filename (without .txt), so plots and
    # reports are labeled with the sample's real name, e.g. "27284 pristine".
    sid = os.path.splitext(os.path.basename(fp))[0]
    return sid, _cache[fp]["meta"], _cache[fp]["df"]
...
def _load_generic_file(filepath: str) -> tuple:
    ...
    # First, try to detect if this is a beamline scan file (header row first column
    # is "Time of Day" or "Time (s)"). If so, use the xas_utils loader
    try:
...
TOOLS = [
        ...
        "function": {
            ...
            "parameters": {
                ...
                "properties": {
                    ...
                    "e_min": {"type": "number", "description": "Minimum energy in eV for the plot range (x-axis zoom). Optional."},
                    "e_max": {"type": "number", "description": "Maximum energy in eV for the plot range (x-axis zoom). Optional."},
                    "y_min": {"type": "number", "description": "Minimum y-axis value for the plot range (y-axis zoom). Optional."},
                    "y_max": {"type": "number", "description": "Maximum y-axis value for the plot range (y-axis zoom). Optional."},
                    "log_scale": {"type": "boolean", "description": "If true, use logarithmic scale on the y-axis. Default false."},
    ...
    },
    {
        ...
        "function": {
            "name": "compare_scans",
            "description": "Overlay multiple scans on one plot for comparison. Works on any data file (raw SigScan, calibrated, renamed, or exported). By default signals are divided by I0 (normalized). Set normalize=false only when the user explicitly asks for 'raw' data. Supports x/y range zoom, vertical offset/scale, per-scan horizontal/vertical shifts, dual-axis mode, and per-curve styling.",
            "parameters": {
                "type": "object",
                "properties": {
        ...
        "function": {
            ...
            "parameters": {
                ...
                "properties": {
                    ...
                    "flatten": {"type": "boolean", "description": "If true (default), flatten the post-edge region for a flat post-edge around 1.0. Set to false to get the standard normalized spectrum without flattening."},
    ...
    },
    {
        ...
        "function": {
            "name": "smooth_scan",
            "description": "Smooth a scan signal with a Savitzky-Golay filter and plot the raw and smoothed curves overlaid. Use this whenever the user asks to 'smooth' a spectrum. This does NOT differentiate. By default it smooths the raw signal; set divide_by_i0=true to smooth signal/I0 instead.",
            "parameters": {
                "type": "object",
                "properties": {
                    "scan_id": {"type": "string", "description": "Scan identifier."},
                    "signal": {"type": "string", "description": "Signal channel (e.g. 'TEY', 'TFY', 'MCP')."},
                    "smooth_window": {"type": "integer", "description": "Savitzky-Golay window size in POINTS (odd integer, larger than polyorder). Larger = smoother. Auto-selected (~5% of points) if not given."},
                    "polyorder": {"type": "integer", "description": "Polynomial order fitted within each window. Default 2 (quadratic)."},
                    "divide_by_i0": {"type": "boolean", "description": "If true, smooth signal/I0. If false (default), smooth the raw signal."},
                },
                "required": ["scan_id", "signal"],
            },
        },
        ...
        "function": {
            ...
            "parameters": {
                ...
                "properties": {
                    ...
                    "e_min": {"type": "number", "description": "Minimum X value for the plot range (x-axis zoom). Optional."},
                    "e_max": {"type": "number", "description": "Maximum X value for the plot range (x-axis zoom). Optional."},
                    "y_min": {"type": "number", "description": "Minimum y-axis value for the plot range (y-axis zoom). Optional."},
                    "y_max": {"type": "number", "description": "Maximum y-axis value for the plot range (y-axis zoom). Optional."},
                    "log_scale": {"type": "boolean", "description": "If true, use logarithmic scale on the y-axis. Default false."},
        ...
        "function": {
            ...
            "parameters": {
                ...
                "properties": {
                    ...
                    "description": {"type": "string", "description": "Description of the event (e.g. 'Major shutdown for maintenance', 'Grating replaced', 'Mirror M1 swapped')."},
                },
...
def tool_compare_scans(scan_ids: list, signal: str = None, signals: list = None,
    ...
    if y_min is not None or y_max is not None:
        cur_ymin, cur_ymax = ax_left.get_ylim()
        ax_left.set_ylim(y_min if y_min is not None else cur_ymin,
                          y_max if y_max is not None else cur_ymax)
...
def tool_normalize_scan(scan_id: str, signal: str, e0: float = None, flatten: bool = True,
...
def tool_smooth_scan(scan_id: str, signal: str, smooth_window: int = None,
                     polyorder: int = 2, divide_by_i0: bool = False, **kw) -> str:
    global _last_plot, _last_plot_b64
    try:
        sid, meta, df = _load(scan_id)
    except FileNotFoundError as e:
    ...
    energy = _get_energy(df)
    if divide_by_i0:
        raw = xu.normalize_by_i0(df, signal)
        ylabel = f"{signal} / I0"
    else:
        raw = np.asarray(xu.get_signal(df, signal), dtype=float)
        ylabel = signal

    smoothed = xu.smooth_signal(raw, window=smooth_window, polyorder=polyorder)

    n = len(raw)
    eff_window = smooth_window if smooth_window else max(7, n // 20)
    if eff_window > n:
        eff_window = n
    if eff_window % 2 == 0:
        eff_window -= 1
    ...
    ax.plot(energy, raw, color="0.65", linewidth=1.0, label=f"{ylabel} (raw)")
    ax.plot(energy, smoothed, "b-", linewidth=1.8,
            label=f"Savitzky-Golay (window={eff_window} pts, order={polyorder})")
    ax.set_xlabel("Energy (eV)")
    ax.set_ylabel(ylabel)
    ax.set_title(f"{sid} — {ylabel} (smoothed)")
    ...
    _pending_images.append(img_b64)
    _last_plot_b64 = img_b64
    _last_plot = {"energy": energy, "signal": smoothed,
                  "signal_name": f"{signal}_smoothed", "scan_id": sid}

    return (
        f"Smoothed {ylabel} for {sid} with a Savitzky-Golay filter "
        f"(window={eff_window} points, polynomial order={polyorder}).\n"
        f"Energy range: {energy.min():.2f}–{energy.max():.2f} eV, {n} pts."
    )
...
Available signal types: TEY (Total Electron Yield), TFY (Total Fluorescence Yield; recorded on the Channeltron channel). MCP may be absent for some scans.
Energy column: photon energy in eV (column "Energy UDP").
...
- plot_scan: Plot a single scan. By default divides signal by I0 (normalized). Use normalize=false only when the user explicitly asks for "raw" data. Supports e_min/e_max to zoom on x-axis, y_min/y_max to zoom on y-axis. Works on any data file.
- compare_scans: Overlay multiple scans on one plot. By default divides signals by I0 (normalized). Use normalize=false only when the user explicitly asks for "raw" data. Works on any data file. Supports:
    * e_min/e_max: zoom into a specific energy range (x-axis)
    * y_min/y_max: zoom into a specific signal range (y-axis)
...
- normalize_scan: Athena-style XAS normalization (pre-edge subtraction + post-edge normalization so edge step = 1). The tool produces TWO outputs: "normalized" (norm) and "flattened" (flat). By default it returns the FLATTENED spectrum (flatten=true), which removes the post-edge slope so the post-edge region is flat around 1.0. Set flatten=false to get the standard normalized spectrum without flattening. The left panel shows the raw signal/I0 with pre-edge and post-edge fit lines; the right panel shows the normalized/flattened result. Works on any data file. Supports save=true to immediately export the normalized data (use this when normalizing multiple scans to avoid the save_data overwrite issue).
...
- smooth_scan: Smooth a signal with a Savitzky-Golay filter (no differentiation) and overlay raw vs smoothed. Use when the user says "smooth". Optional smooth_window (odd, in points) and polyorder (default 2); divide_by_i0 to smooth signal/I0.
...
- When the user asks to "zoom in" or specifies an energy range (e.g. "plot from 520 to 560 eV"), use e_min/e_max parameters for x-axis zoom
- When the user asks to zoom on the y-axis or specifies a signal/intensity range (e.g. "set y range from 0 to 5", "zoom y to 0.5-1.5", "set ymin to 0", "set ymax to 10"), use y_min/y_max parameters. These work on plot_scan, compare_scans, average_scans, plot_file, and compare_files.
- When the user asks for "log scale", "logarithmic scale", "log y", or "plot in log", use log_scale=true. This sets the y-axis to logarithmic scale. Works on plot_scan, compare_scans, average_scans, plot_file, and compare_files. Can be combined with y_min/y_max.
...
- When the user says "smooth", "smoothed", "denoise", or "Savitzky-Golay", use smooth_scan (not derivative_scan); bigger smooth_window = smoother
...
- normalize_scan divides by I0 first (if available), then does Athena-style pre-edge subtraction and post-edge normalization (edge step normalized to 1). Two output modes: default "flattened" (flatten=true) subtracts pre-edge line, divides by edge step, AND removes the post-edge slope so the post-edge region is flat around 1.0; "normalized only" (flatten=false) does the same but without flattening the post-edge. When the user just says "normalize", use the default (flatten=true). When they explicitly ask for "unflattened" or "without flattening", use flatten=false.
...
async function loadFileTree() {
  ...
  try {
    ...
    // Render each root directory (801_Data, exported_data, etc.)
