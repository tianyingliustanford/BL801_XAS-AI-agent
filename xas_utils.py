...
# Each physical quantity can appear under different column names. We list candidate
# names per quantity and resolve to whichever one is present in the file, so the same
# code reads files saved with different column headers.
ENERGY_COL_CANDIDATES = [
    "Energy UDP",
    "Mono eV Calib high E Grating",   # alternate grating naming
    "Mono eV Calib low E Grating",    # alternate grating naming
    "Energy_eV",                      # exported two-column files
]

SIGNAL_COLUMN_CANDIDATES = {
    "TEY": ["TEY", "TEY UHV XAS"],
    "TFY": ["Channeltron", "TFY UHV XAS", "TFY"],   # the Channeltron channel is used as TFY
    "MCP": ["MCP Np", "MCP"],                         # not present in all scans
    "I0":  ["Izero", "I0"],                           # some scans label I0 as "Izero"
}

# Backward-compatible single-name aliases (first candidate of each)
TEY_COL = SIGNAL_COLUMN_CANDIDATES["TEY"][0]
TFY_COL = SIGNAL_COLUMN_CANDIDATES["TFY"][0]
I0_COL = SIGNAL_COLUMN_CANDIDATES["I0"][0]
MCP_COL = SIGNAL_COLUMN_CANDIDATES["MCP"][0]
SIGNAL_COLUMNS = {k: v[0] for k, v in SIGNAL_COLUMN_CANDIDATES.items()}
...
def _detect_header_lines(filepath: str) -> int:
    ...
    Scans for the column-header line. Its first column may be 'Time of Day' or
    'Time (s)' depending on scan mode; both start with 'Time' and contain a tab,
    while no metadata line does. Returns the 0-indexed row number of that line, so
    skiprows=N skips everything before it. Returns -1 if not found (not a scan file).
    """
    with open(filepath, "r") as f:
...
def extract_scan_number(text: str) -> str | None:
    """Pull the unique scan number out of a query or filename.

    Handles '27284', '27284 pristine', 'SigScan27284', 'SigScan27284 pristine.txt',
    and subdirectory-prefixed forms like '260401/27310 Fe2O3 ref'. Digits right after
    'SigScan' win; otherwise the first run of >=3 digits in the basename is used (so a
    date-coded folder like 260401/ is not mistaken for the scan number).
    """
    if not text:
        return None
    m = re.search(r"SigScan\s*(\d+)", text, re.IGNORECASE)
    if m:
        return m.group(1)
...
def scan_filepath(scan_id: str, data_dir: str = DATA_DIR) -> str:
    ...
    # Third pass: match by the unique scan number, so renamed files such as
    # "27284 pristine.txt" (no SigScan prefix) are still found by number alone.
    number = extract_scan_number(scan_id)
    if number:
        token = re.compile(rf"(?<!\d){re.escape(number)}(?!\d)")
        matches = [os.path.join(r, f)
                   for r, _d, fs in os.walk(data_dir)
                   for f in fs
                   if f.lower().endswith(".txt") and token.search(Path(f).stem)]
        if matches:
            matches.sort(key=lambda p: (len(Path(p).stem), p))
            return matches[0]
...
def list_scan_files(data_dir: str = DATA_DIR, date_filter: str | None = None) -> list[str]:
    ...
    e.g. '260401/SigScan45650' for files in 801_Data/260401/.
    ...
    for root, _dirs, files in os.walk(data_dir):
        ...
        for f in files:
            if f.endswith(".txt") and (f.startswith("SigScan") or extract_scan_number(f)):
                stem = Path(f).stem
                if rel == ".":
...
def _resolve_signal_col(df: pd.DataFrame, signal_name: str) -> str | None:
    """Resolve a logical signal (TEY/TFY/MCP/I0) to a column present in this file,
    trying the candidate aliases in order. Returns None if none are present."""
    for col in SIGNAL_COLUMN_CANDIDATES.get(signal_name.upper(), []):
        if col in df.columns:
            return col
    return None
...
def get_signal(df: pd.DataFrame, signal_name: str) -> np.ndarray:
    # Try the alias candidates first (e.g. TFY -> "Channeltron" or "TFY UHV XAS")
    col = _resolve_signal_col(df, signal_name)
    if col is not None:
    ...
    for c in df.columns:
        if c.lower() == sig_lower:
            return df[c].values
    # Flexible matching: column whose name starts with the signal name
    ...
    if signal_name.upper() not in SIGNAL_COLUMN_CANDIDATES:
        raise ValueError(f"Unknown signal '{signal_name}'. Choose from {list(SIGNAL_COLUMN_CANDIDATES.keys())} or available columns: {list(df.columns)}")
    raise KeyError(f"No column for signal '{signal_name}'. Tried {SIGNAL_COLUMN_CANDIDATES[signal_name.upper()]}. Available: {list(df.columns)}")


def normalize_by_i0(df: pd.DataFrame, signal_name: str) -> np.ndarray:
    sig = get_signal(df, signal_name)
    # Only divide if a dedicated I0 column exists (two-column exported files are
    # already normalized, so skip I0 division).
    i0_col = _resolve_signal_col(df, "I0")
...
def list_available_signals(df: pd.DataFrame) -> list[str]:
    return [label for label in SIGNAL_COLUMN_CANDIDATES
            if _resolve_signal_col(df, label) is not None]
...
def smooth_signal(
    ...
    """Savitzky-Golay smoothing of a 1-D signal (no differentiation).

    Fits a low-order polynomial to a sliding window and replaces the centre point
    with the fitted value. Smooths noise while preserving peak position and height.

    Parameters
    ----------
    mu : array
        Signal to smooth (raw channel, or signal/I0).
    window : int or None
        Window size in points (odd, > polyorder). Auto ~5% of length (min 7) if None.
        Larger = smoother.
    polyorder : int
        Polynomial order within each window (default 2). Must be < window.
    """
    n = len(mu)
    if window is None:
        window = max(7, n // 20)
    ...
    if window % 2 == 0:
        window -= 1
    if window < 3:
        window = 3 if n >= 3 else n
    if polyorder >= window:
        polyorder = window - 1
    if polyorder < 1:
        polyorder = 1
    return savgol_filter(mu, window_length=window, polyorder=polyorder, deriv=0)
